...
// 🛡️ SECURITY PATCH (High Risk Fix): XSS Protection - HTML ট্যাগ নিষ্ক্রিয় করার ফাংশন
...
export function runOmLang(code) {
  // Strict Input Validation (Type & Empty Check)
  if (!code || typeof code !== 'string' || code.trim() === '') return '';
  
  // Basic Code Sanitization (Preventing JS/DOM injection)
  const maliciousKeywords = ['document.', 'window.', 'localStorage', '<script>'];
    ...
    for (let i = 0; i < blockLines.length; i++) {
      ...
      try {
        ...
        if (line.startsWith('show')) {
          ...
          // 🛡️ SECURITY PATCH: Sanitize output before pushing to screen

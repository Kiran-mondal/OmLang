...
export function runOmLang(code) {
  ...
  let output = [];
  let globals = {}; // 🌟 মূল মেমরি
  let functions = {};

  const lines = code.split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('//'));

  // 🌟 executeBlock এখন দুটি জিনিস নেবে: কোড ব্লক এবং লোকাল মেমরি (যদি থাকে)
  const executeBlock = (blockLines, locals = null) => {
    let skipElse = false; 

    // Helper: বর্তমান স্কোপের সব ভেরিয়েবল একসাথে পাওয়ার জন্য
    ...
    for (let i = 0; i < blockLines.length; i++) {
      ...
      if (standaloneFuncMatch && functions[standaloneFuncMatch[1]] && !line.startsWith('show') && !line.includes('=')) {
          ...
          let funcLocals = {}; // 🌟 নতুন লোকাল মেমরি তৈরি হচ্ছে
      ...
      if (line.includes('=') && !line.startsWith('show') && !line.startsWith('if') && !line.startsWith('maybe') && !line.startsWith('while')) {
        ...
        // 🌟 যদি ফাংশনের ভেতর থাকি, তবে লোকাল মেমরিতে সেভ হবে, নাহলে গ্লোবালে

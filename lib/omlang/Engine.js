 ...
 * Bug Fixed: 'else' blocks now execute perfectly when 'if' is false.
 */
...
export function runOmLang(code) {
    ...
    let memory = {};
    let forLoops = {}; 
    let skipElse = false; // NEW: এই ফ্ল্যাগটি if/else এর ট্র্যাক রাখবে
    ...
    while (i < lines.length) {
        ...
        try {
            ...
            if (ifMatch) {
                ...
                if (result) {
                    i++; 
                    skipElse = true; // Condition true: পরের else স্কিপ করতে হবে
                } else {
                    i = blockEnds[i] + 1; 
                    skipElse = false; // Condition false: পরের else রান করতে হবে
                }
                continue;
            }
            ...
            if (line === 'else {') {
                if (skipElse) {
                    i = blockEnds[i] + 1; // if সত্য ছিল, তাই else স্কিপ হবে
                    skipElse = false; // রিসেট
                } else {
                    i++; // if মিথ্যা ছিল, তাই else-এর ভেতর ঢুকবে
                }
                continue;
            }
